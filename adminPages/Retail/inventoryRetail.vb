...
Public Class inventoryRetail
    ...
    Private Sub InitializeArchivedProductsButton()
        ...
        btnArchivedProducts.Location = New Point(TextBoxSearch.Right + 80, TextBoxSearch.Top + 135)
    ...
    Private Sub btnArchivedProducts_Click(sender As Object, e As EventArgs) Handles btnArchivedProducts.Click
        ' Pass "Retail" as the initial product type
        Dim archivedForm As New ArchivedProductsForm("Retail")
    ...
    Private Sub btnDeleteBatch_Click(sender As Object, e As EventArgs) Handles btnDeleteBatch.Click
        ...
        Try
            ...
            Using conn As New SqlConnection(GetConnectionString())
                conn.Open()
                Dim getProductIDQuery As String = "SELECT ProductID FROM retailProducts WHERE SKU = @SKU"
            ...
            If ProductHasSalesRecords(productID) Then
                ...
                If archiveResult = DialogResult.Yes Then
                    ...
                    Using conn As New SqlConnection(GetConnectionString())
                        conn.Open()
                        Dim archiveQuery As String = "UPDATE retailProducts SET IsArchived = 1, ArchivedDate = GETDATE(), ArchivedBy = @ArchivedBy, ArchiveReason = @ArchiveReason WHERE SKU = @SKU"
                ...
                Using conn As New SqlConnection(GetConnectionString())
                    ...
                    Using transaction = conn.BeginTransaction()
                        Try
                            ...
                            ' Delete the batch
                            Dim query As String = "DELETE FROM retailProducts WHERE SKU = @SKU"
...
SELECT
 ...
 p.unit,
   ...
   INNER JOIN Categories c ON p.CategoryID = c.CategoryID
    ...
    p.expirationDate ASC"
        ...
        Try
            ...
            With tableDataGridView
                ...
                If .Columns.Contains("QR Code Hex (Dbl Click)") Then
                    .Columns("QR Code Hex (Dbl Click)").HeaderText = "QR Code (Click to view)"
                    .Columns("QR Code Hex (Dbl Click)").Visible = True
                End If

                ' Hide the binary QR image column
    ...
    Private Sub TableDataGridView_RowClick(sender As Object, e As DataGridViewCellEventArgs)
        ...
        Try
            ...
            Else
                ...
                If e.ColumnIndex >= 0 AndAlso tableDataGridView.Columns(e.ColumnIndex).Name = "QR Code Hex (Dbl Click)" Then
    ...
    Private Sub TableDataGridView_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs)
        ...
        Try
            ...
            If isShowingDetailView Then
                ...
                Using conn As New SqlConnection(GetConnectionString())
                    conn.Open()
                    Dim query As String = "SELECT ProductID FROM retailProducts WHERE SKU = @SKU"
                    Using cmd As New SqlCommand(query, conn)
                        cmd.Parameters.AddWithValue("@SKU", sku)
                ...
                Dim editForm As New editItemRetail(Me)
    ...
    ' Remove old CellDoubleClick handler - now using CellClick for row selection
    'Private Sub tableDataGridView_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles tableDataGridView.CellClick
    '    ' REMOVED - replaced with TableDataGridView_RowClick
    'End Sub
